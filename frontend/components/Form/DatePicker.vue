<template>
  <div v-if="!inline" class="form-control w-full">
    ...
    <input ref="input" v-model="selected" type="date" class="input input-bordered w-full" aria-label="date"/>
  </div>
  <div v-else class="sm:grid sm:grid-cols-4 sm:items-start sm:gap-4">
    <label class="label">
      <span class="label-text"> {{ label }} </span>
    </label>
    <input v-model="selected" type="date" class="input input-bordered col-span-3 w-full mt-2" aria-label="date"/>
  </div>
</template>
